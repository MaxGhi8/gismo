...
\cite[\S4.4]{kleiss2012ieti}, \cite[Ch.~6]{pechstein2013finite} is:
\begin{equation}
...
\begin{itemize}
  \item[(i)] \textbf{Refinement} ($K$ fixed): $H/h\sim(N/K)^{1/2}$
    gives $\kappa=O(\log^{2}N)$ and $O(\log N)$ CG iterations.
    With direct local solvers the serial total cost is
    $O(N^{3/2}K^{-1/2}\log N)$.
  ...
  \item[(iii)] \textbf{Degree} ($h$ fixed): $C=O(p^{3})$~\cite[\S4.4]{kleiss2012ieti},
    algebraic growth far milder than the super-algebraic
    $\kappa(A_h)\leq C\,p^{2d+2}4^{pd}$ for unpreconditioned
    CG~\cite[Thm.~4.8]{gahalaut2019condition}.
\end{itemize}
...
(Figure~\ref{fig:yeti-iters-r}), confirming $h$-robustness on
the flat domain.
...
(Figure~\ref{fig:teapot-iters-r}), far from the $O(1)$ prediction for
...
grow from $16$ to only $25$ (Figure~\ref{fig:teapot-iters-p}), confirming
