...
\begin{table}[h]
  \centering
  \caption{Predicted asymptotic scaling for the refinement sweep ($K$, $p$ fixed),
    with $N = K n_k$.
  ...
  \begin{tabular}{lcccc}
    ...
    CG + Jacobi                    & $O(n_k)$            & $O(n_k^{1/2})$     & $O(K\,n_k^{3/2})$              & ---    \\
    CG + symm.\ GS                 & $O(n_k)$            & $O(n_k^{1/2})$     & $O(K\,n_k^{3/2})$              & ---                      \\
    CG + Multigrid                 & $O(1)$              & $O(1)$             & $O(K\,n_k)$                    & ---                      \\
...
\begin{itemize}
  \item[(i)] \textbf{Refinement} ($K$ fixed): $H/h\sim(N/K)^{1/2}$
    gives $\kappa=O(\log^{2}N)$ and hence $O\!\left(\sqrt{\kappa}\right)=O(\log N)$ CG iterations.
...
\begin{table}[t]
    ...
    \begin{tabular}{lcccc}
        ...
        GMRES + symmetric Gauss--Seidel & 0.13 & 4.57  & 100  & yes \\
        CG (no preconditioner)          & 0.13 & 0.47  & 446  & yes \\
        CG + Jacobi                     & 0.13 & 0.46  & 319  & yes \\
        CG + symmetric Gauss--Seidel    & 0.13 & 3.23  & 104  & yes \\
...
\begin{table}[t]
    ...
    \begin{tabular}{lcccc}
        ...
        GMRES + symmetric Gauss--Seidel & 0.015 & 0.21   & 134  & yes \\
        CG (no preconditioner)          & 0.015 & 0.10   & 1000 & no  \\
        CG + Jacobi                     & 0.015 & 0.053  & 381  & yes \\
        CG + symmetric Gauss--Seidel    & 0.015 & 0.16   & 144  & yes \\
