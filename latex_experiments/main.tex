...
of patch $k$: the $i$-th row collects the physical coordinates of the Greville
abscissa~\cite{cottrell2009isogeometric} of local basis function $i$,
$\gmapk{k}(\vect{\eta}^{\,\mathrm{Gr}}_i) \in \R^D$, and its NURBS weight $w_i$,
\begin{equation}
    \vect{\zeta}_k
...
network.
$M_\theta(\vect{\zeta}_k)$ is parameterized in the SPD factorized form
\begin{equation}
...
The matrix $\dv^{-1/2} Q_\theta Q_\theta^{\top} + \varepsilon_\theta I$ is SPD
for any $Q_\theta$ and $\varepsilon_\theta > 0$.
...
encoder of~\cite{moller2026iganet}\mg{fix citation once available}: the rows of $\vect{\zeta}_k$ are treated as
a sequence of $n_k$ tokens in $\R^{D+1}$, processed through a learnable affine
embedding plus sinusoidal positional encoding~\cite{vaswani2017attention},
followed by $L$ layers of multi-head self-attention.
A cross-attention resampling step then maps the encoded sequence to
$G_\theta \in \R^{|\skel_k| \times \dv}$ using $|\skel_k|$ learnable target
queries, and a final linear projection gives $Q_\theta = G_\theta W_Q$.
We refer to~\cite{moller2026iganet} for the complete architecture details.

\begin{rmk}[PCG compatibility]
\label{rmk:linearity}
Since $\vect{v} \notin \mathrm{input}(\theta)$, the preconditioner
$\Pneural$ in~\eqref{eq:neural-prec} is a linear SPD operator for every $\theta$,
so standard PCG applies without modification.
\end{rmk}
...
\label{subsec:pcg-integration}

The operator~\eqref{eq:neural-schur} enters IETI-DP only through
$\Pneural$~\eqref{eq:neural-prec}; the system~\eqref{eq:ieti-schur} is solved
exactly. Approximation error in $\Snn$ therefore does not perturb $\lagmult$
or $u_h$ beyond the solver tolerance; it affects only the PCG iteration count.
Performance is measured by iteration counts and effective condition numbers of
$\Pneural\Fschur$, not by solution accuracy.

Since $M_\theta(\vect{\zeta}_k)$ in~\eqref{eq:spd-param} is SPD for all
$\theta$, $\Pneural$ is SPD and PCG applies. The geometry descriptors
$\{\vect{\zeta}_k\}$ are assembled once and reused at every iteration.

\begin{rmk}[Per-iteration cost]
\label{rmk:cost-neural}
Each application of $\Pneural$ requires $\npatch$ network evaluations at cost
$O(|\skel_k|\,\dv)$ each, independent of $n_{k,I} = n_k - |\skel_k|$ and of
$\kappa(A^{(k)}_{II})$.
The exact preconditioner~\eqref{eq:scaled-dirichlet} requires $\npatch$
triangular solves scaling as $O(n_{k,I}^{1.5})$ in two dimensions.
\end{rmk}
...
Because $d = D = 2$, the manufactured solution $u = \sin(x)\cos(y)$,
$f = 2\sin(x)\cos(y)$, is exact and the $L^2$ error is meaningful.
...
\begin{table}[t]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        \textbf{Direct Cholesky}                &  \textbf{0.06} & -    & yes \\
